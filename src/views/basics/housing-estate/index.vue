<template>
  <div class="app-container">
    <div class="filter-container">
      <span>快速查询：</span>
      <el-input v-model="searchKey" class="input-search" placeholder="请输入小区名称" clearable>
        <i slot="prefix" class="el-input__icon el-icon-search" />
      </el-input>

      <el-button type="primary" @click="onLoadHousingEstateData">搜索</el-button>
      <el-button @click="onReset">重置</el-button>
      <div class="flex-1" />
      <el-button type="success" @click="onHousingEstateAdd">新增</el-button>
    </div>
    <el-table :key="housingEstateListKey" v-loading="housingEstateListLoading" :data="housingEstateList" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column prop="houseContractorName" label="小区联系人" min-width="120" />
      <el-table-column prop="operationsName" label="运维负责人" min-width="120" />
      <el-table-column prop="operationsPhone" label="运维负责人电话" min-width="120" />
      ...
      <el-table-column fixed="right" prop="operation" label="操作" min-width="250">
        <template slot-scope="scope">
          <el-button size="mini" @click="onHousingEstateEdit(scope.$index, scope.row)">编辑</el-button>
          <el-button size="mini" type="primary" @click="onShowAddBikeShedDialog(scope.$index, scope.row)">新增车棚</el-button>
          <el-button size="mini" type="danger" @click="onHousingEstateDelete(scope.$index, scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>

    <pagination v-show="total > 0" :total="total" :page.sync="pageNo" :limit.sync="pageSize" @pagination="onLoadHousingEstateData" />

    <el-dialog title="新增小区" :visible.sync="isShowDialogAddHousingEstate">
      <el-form ref="addHousingEstateRef" :rules="addHousingEstateRules" :model="NewHousingEstate" label-position="right" label-width="140px" style="width: 400px; margin-left: 50px">
        <el-form-item label="小区所在省：" prop="province">
          <el-select v-model="NewHousingEstate.province" class="filter-item" placeholder="请选择所在省份" :disabled="dialogStatus == 'update'" @change="onProvinceSelectChange">
            <el-option v-for="item in provinceList" :key="item.name" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="小区所在市：" prop="city">
          <el-select v-model="NewHousingEstate.city" class="filter-item" placeholder="请选择所在市" :disabled="dialogStatus == 'update'" @change="onCitySelectChange">
            <el-option v-for="item in cityList" :key="item.name" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="小区所在区：" prop="region">
          <el-select v-model="NewHousingEstate.region" class="filter-item" :disabled="dialogStatus == 'update'" placeholder="请选择所在区">
            <el-option v-for="item in regionList" :key="item.name" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="小区名字：" prop="houseName">
          <el-input v-model="NewHousingEstate.houseName" placeholder="请输入小区名字" clearable />
        </el-form-item>
        <el-form-item label="小区地址：" prop="houseAddr">
          <el-input v-model="NewHousingEstate.houseAddr" placeholder="请输入小区地址" clearable />
        </el-form-item>
        <el-form-item label="小区联系电话：" prop="housePhone">
          <el-input v-model="NewHousingEstate.housePhone" placeholder="请输入小区联系电话" maxlength="12" clearable />
        </el-form-item>
        <el-form-item label="小区负责人：" prop="houseContractorName">
          <el-input v-model="NewHousingEstate.houseContractorName" placeholder="请输入小区负责人姓名" clearable />
        </el-form-item>
        <el-form-item label="运维负责人：" prop="operationsName">
          <el-input v-model="NewHousingEstate.operationsName" placeholder="请输入运维负责人姓名" clearable />
        </el-form-item>
        <el-form-item label="运维负责人电话：" prop="operationsPhone">
          <el-input v-model="NewHousingEstate.operationsPhone" placeholder="请输入运维负责人电话" maxlength="12" clearable />
        </el-form-item>
        <el-form-item label="备注：" prop="bak">
          <el-input v-model="NewHousingEstate.bak" placeholder="请输入备注" :autosize="{ minRows: 2, maxRows: 4 }" type="textarea" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogStatus === 'add'? onAddHousingEstate(): onUpdateHousingEstate()">
          添加
        </el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog title="新增车棚" :visible.sync="isShowDialogAddBikeShed">
      <el-form ref="addBikeShedRef" :rules="addBikeShedRules" :model="NewBikeShed" label-position="right" label-width="140px" style="width: 400px; margin-left: 50px">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="isShowDialogAddBikeShed = false"> 取消 </el-button>
        <el-button type="primary" @click="onAddBikeShed()"> 添加 </el-button>
      </div>
    </el-dialog>
  </div>
</template>
